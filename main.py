...
with ui.header(elevated=True).style('background-color: #616161; padding: 16px; postion:relative;') as header:
    with ui.row().classes('items-center'):
        ...
        ui.markdown('Finanz- und Pensionierungsplaner *by Tim Wüthrich*').classes('text-h3 q-ml-md').style('color: white;')
with ui.footer().style('background-color: #616161; padding: 16px;'):
    with ui.row().classes('items-center'):
     ui.markdown("Jegliche gewerbliche Verwendung von Dritten ist untersagt!").classes('text-body2 q-ml-md')
